...
// - Extra boolean (below) to decide whether to record details of each step played. true = keep details, false = summary only

bool record_Every_Step(true);

// A.Oram 2016, based on the P.Vacher's skeletal program from first year FoP.
...
+             .  -           +   MSG:
+         -   .     -        +
+             .        -  -- +   SLITHERED OVER 0 PELLETS SO FAR!
+             .              +
++++++++++++++++++++++++++++++
___________________________________________________________________________
...
const int MLEFT(SIZEX + 3);				//define left margin for messages (avoiding garden)
...
int __cdecl main()
{
	...
	string message;							// NEW moved here to allow use in SaveTimes function
	...
	while ((newGame | 0x20) != QUIT)		// keep playing games
	{
		...
		while (((key | 0x20) != QUIT) && snailStillAlive && !fullOfLettuce)	//user not bored, and snail not dead or full
		{
			...
			clearMessage(message);						// NEW reset message array, moved from above (so we an use it above in saveTimes)
